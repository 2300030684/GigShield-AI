...
export const ZeroTouchOverlay = ({ trigger, onComplete }) => {
  ...
  const runProcess = async () => {
    const result = await TrustpayZeroTouch.processClaimSteps(
      ...
      (pct) => {
        setProgress(pct);
        if (pct < 100) setProgressLabel(`Processing ${Math.round(pct)}%...`);
        else setProgressLabel('Initiating UPI transfer...');
      }
    );
    ...
    <div className="zt-overlay">
      <div className="zt-card">
        ...
        <div className="zt-header">
          ...
          <h2 className="zt-title">Zero-Touch Claim</h2>
          <p className="zt-subtitle">Processing automatically — no action needed</p>
        </div>

        {!claimResult && (
          <div className="zt-steps">
            {[
              { id: 1, name: 'Disruption Verified',     icon: '📡' },
              { id: 2, name: 'Location Auto-Detected',  icon: '📍' },
              { id: 3, name: 'Earnings Impact Calc.',   icon: '📊' },
              { id: 4, name: 'Fraud Check Passed',      icon: '🛡️' },
              { id: 5, name: 'Policy Coverage Applied', icon: '💳' },
            ].map(s => {
              const completed = completedSteps.find(cs => cs.id === s.id);
              return (
                <div key={s.id} className={`zt-step ${completed ? 'zt-step-done' : ''}`}>
                  <div className={`zt-step-icon ${completed ? 'complete' : 'pending'}`}>
                    {completed ? '✅' : <span className="zt-spinner-mini"></span>}
                  </div>
                  <div className="zt-step-content">
                    <span className="zt-step-name">{s.name}</span>
                    <span className="zt-step-detail">{completed ? completed.detail : 'Waiting...'}</span>
                  </div>
                </div>
              );
            })}
        ...
        {claimResult && (
          <div className="zt-payout-result">
            ...
            <div className="zt-approved-label">CLAIM APPROVED</div>
            <div className="zt-amount">₹{countUp}</div>
            <div className="zt-upi">Sending to {user.upiID || 'your UPI'}</div>
            <div className="zt-claimid">Claim ID: {claimResult.claimID}</div>
          </div>
        )}
      </div>
    </div>
